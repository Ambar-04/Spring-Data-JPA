...
//        List<Product> products = productRepository.findByNameLike("%product%"); //When you use LIKE in a SQL query, it looks for exact matches unless you include wildcard characters. The % symbol is commonly used as a wildcard character, representing zero or more characters
    ...
    void findTop2ByOrderByPriceDescMethod(){
        ...
        //List<Product> products = productRepository.findFirst2ByOrderByPriceDesc(); //same output
        products.forEach((p) ->{
            System.out.println(p.getId());
            System.out.println(p.getName());
        });
